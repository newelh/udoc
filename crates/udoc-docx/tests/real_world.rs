//! Real-world DOCX file extraction tests.
//!
//! Tests against actual .docx files from public sample repositories to validate
...
fn real_world_no_panics_on_all_files() {
    ...
    for entry in entries {
        ...
        if path.extension().map(|e| e == "docx").unwrap_or(false) {
            ...
            match DocxDocument::from_bytes(&data) {
                Ok(mut doc) => {
    ...
    eprintln!("processed {docx_count} .docx file(s)");
}
...
fn real_world_sample_files_text_extraction() {
    ...
    for entry in std::fs::read_dir(&dir).expect("read corpus dir") {
        ...
        if path.extension().map(|e| e == "docx").unwrap_or(false) {
            ...
            if let Ok(mut doc) = DocxDocument::from_bytes(&data) {
