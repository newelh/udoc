...
mod tests {
    ...
    /// Parsed inputs to the generic-region decoder extracted from a
    /// test fixture PDF.
    ...
    /// Load the arith-coded payload of the page-2 immediate-generic
    /// region from the govdocs1 fixture. Offsets were verified at
    /// fixture authoring time and cross-checked in
    /// `segments::fixture_generic_first_two_headers_match_expected`.
    fn load_govdocs_generic_fixture() -> GenericFixture {
        const FIXTURE: &[u8] =
            include_bytes!("../../../../../tests/corpus/goldens/jbig2/generic/govdocs1-008244.pdf");
        const STREAM_START: usize = 18640;
        const STREAM_LEN: usize = 5252;
        let stream = &FIXTURE[STREAM_START..STREAM_START + STREAM_LEN];

        // seg 0 (page_info) : 11 hdr + 19 data = 30 bytes
        // seg 1 (immed_generic) : 11 hdr + region header + flags + AT + arith data
        let mut off = 11 + 19 + 11;

        // Region segment information field (§7.4.1): W(4) H(4) X(4) Y(4) extcomb(1)
        ...
        let flags = stream[off];
        off += 1;
        let mmr = (flags & 0x01) != 0;
        assert!(!mmr, "fixture must use arith path");
        let template = (flags >> 1) & 0x3;
        let tpgdon = (flags & 0x08) != 0;

        // AT pixel bytes follow for non-MMR
        let n_at = if template == 0 { 4 } else { 1 };
        let mut at = Vec::with_capacity(n_at);
        for _ in 0..n_at {
            let x = stream[off] as i8;
            let y = stream[off + 1] as i8;
            at.push((x, y));
            off += 2;
        }

        let arith_data = stream[off..].to_vec();
        GenericFixture {
            width: w,
            height: h,
            gbtemplate: template,
    ...
    #[test]
    fn fixture_generic_region_decodes_byte_exact() {
        // End-to-end on the govdocs1 fixture. Validates the full
        // arith -> generic region pipeline against a pre-committed
        // byte-exact snapshot hash.
        //
        // The snapshot hash was computed at authoring time
        // from the hayro_jbig2 0.3.0 decoder (which is itself
        // byte-exact against MuPDF per the `tools/jbig2-validate`
        // end-to-end gate). Any regression in our decoder's context
        // computation, AT-pixel handling, or TPGDON logic flips the
        // hash and fails this test.
        //
        // The byte-count (black/white split) acts as a human-readable
        // canary: if the hash changes, check whether the counts also
        // changed to rule out dimension-mismatch corruption.
        const EXPECTED_WIDTH: u32 = 2044;
        const EXPECTED_HEIGHT: u32 = 1513;
        const EXPECTED_HASH: u64 = 0x3604_1ec7_dc00_ec2e;
        const EXPECTED_BLACK: usize = 113_927;
        const EXPECTED_WHITE: usize = 2_978_645;

        let fixture = load_govdocs_generic_fixture();
        assert_eq!(fixture.width, EXPECTED_WIDTH);
        assert_eq!(fixture.height, EXPECTED_HEIGHT);
        assert_eq!(fixture.gbtemplate, 0, "fixture uses Template 0");

        let params = GenericRegionParams {
            width: fixture.width,
            height: fixture.height,
            gbtemplate: fixture.gbtemplate,
            tpgdon: fixture.tpgdon,
            mmr: false,
            at_pixels: fixture.at_pixels.clone(),
        };
        let mut arith = ArithDecoder::new(&fixture.arith_data);
        let bitmap = decode_generic_region(&fixture.arith_data, &params, Some(&mut arith))
            .expect("fixture generic region decodes");
        assert_eq!(
            bitmap.len(),
            (fixture.width as usize) * (fixture.height as usize)
        );

        let black = bitmap.iter().filter(|&&b| b == 0x00).count();
        let white = bitmap.iter().filter(|&&b| b == 0xFF).count();
        assert_eq!(
            black + white,
            bitmap.len(),
            "every pixel must be 0x00 or 0xFF"
        );
        assert_eq!(black, EXPECTED_BLACK, "black pixel count drifted");
        assert_eq!(white, EXPECTED_WHITE, "white pixel count drifted");

        // FNV-1a 64 as a cheap byte-exact snapshot. No extra dependency.
        ...
        assert_eq!(
            hash, EXPECTED_HASH,
            "byte-exact snapshot drift (FNV-1a-64). If this was an intentional \
             fix, regenerate the hash via the jbig2-smoke-test harness at \
