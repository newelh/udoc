...
mod tests {
    ...
    /// Extract the page-2 JBIG2Decode stream from the fixture PDF. The
    /// offset was sanity-checked at fixture authoring time; a helper in
    /// (jbig2-validate) does the same lookup with a real PDF
    /// parser.
    fn extract_fixture_jbig2_stream() -> Vec<u8> {
        const FIXTURE: &[u8] =
            include_bytes!("../../../../tests/corpus/goldens/jbig2/generic/govdocs1-008244.pdf");
        // JBIG2 stream for page 2, verified via a pre-committed hex dump.
        // Offsets: start=18640, length=5252. The stream starts with the
        // page-information segment (type 48).
        const START: usize = 18640;
        const LEN: usize = 5252;
        FIXTURE[START..START + LEN].to_vec()
    }

    #[test]
    fn fixture_generic_first_two_headers_match_expected() {
        let stream = extract_fixture_jbig2_stream();
        let mut off = 0;

        let h1 = parse_header(&stream, &mut off).expect("fixture seg 0 parses");
        assert_eq!(h1.segment_number, 0);
        assert_eq!(h1.segment_type, SegmentType::PageInformation);
        assert_eq!(h1.page_association, 1);
        assert_eq!(h1.data_length, 19);
        assert_eq!(h1.header_length, 11);
        // Advance past page-info data.
        off += h1.data_length as usize;

        let h2 = parse_header(&stream, &mut off).expect("fixture seg 1 parses");
        assert_eq!(h2.segment_number, 1);
        assert_eq!(h2.segment_type, SegmentType::ImmediateGenericRegion);
        assert_eq!(h2.page_association, 1);
        assert_eq!(h2.data_length, 5211);

        // Sanity: the two segments account for the whole stream
        // (11 header + 19 data + 11 header + 5211 data = 5252 bytes).
        assert_eq!(off + h2.data_length as usize, stream.len());
    }

    #[test]
    fn fixture_header_roundtrips_to_bytes() {
        // The parsed header re-emitted as bytes must match the original
        // slice. Demonstrates we didn't drop or reinterpret any field.
        let stream = extract_fixture_jbig2_stream();
        let mut off = 0;
        let h1 = parse_header(&stream, &mut off).expect("parses");
        let emitted = emit_header(&h1);
        assert_eq!(emitted, stream[0..h1.header_length]);
    }

    /// Minimal re-emitter for header round-trip coverage. Covers only
    /// the shape used by the fixture (1-byte PA, short-form referred
    /// count <= 4).
    fn emit_header(h: &SegmentHeader) -> Vec<u8> {
        let mut out = Vec::new();
        out.extend_from_slice(&h.segment_number.to_be_bytes());
        let mut flags = h.segment_type.code() & 0x3F;
        // Decide PA4 bit from header_length vs expected short-form size.
        let short_form_len = 4
            + 1
            + 1
            + (h.referred_segments.len() * h.referred_segment_field_width.bytes())
            + 1
            + 4;
        let pa4 = h.header_length != short_form_len;
        if pa4 {
            flags |= 0x40;
        }
        if h.deferred_non_retain {
            flags |= 0x80;
        }
        out.push(flags);
        let count = h.referred_segments.len() as u8;
        assert!(count <= 4, "emit_header test helper is short-form only");
        out.push((count << 5) | (h.retain_flags & 0x1F));
        for &r in &h.referred_segments {
            match h.referred_segment_field_width {
                ReferredFieldWidth::One => out.push(r as u8),
                ReferredFieldWidth::Two => out.extend_from_slice(&(r as u16).to_be_bytes()),
                ReferredFieldWidth::Four => out.extend_from_slice(&r.to_be_bytes()),
            }
        }
        if pa4 {
            out.extend_from_slice(&h.page_association.to_be_bytes());
        } else {
            out.push(h.page_association as u8);
        }
        out.extend_from_slice(&(h.data_length as u32).to_be_bytes());
        out
    }
